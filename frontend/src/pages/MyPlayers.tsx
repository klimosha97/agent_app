 ...
 * Показывает игроков, добавленных в список MY
 */
...
export const MyPlayers: React.FC = () => {
  ...
  const { data, isLoading } = useQuery(
    ['watched-players', 'MY'],
    () => apiService.getWatchedPlayers('MY'),
  );

  const removeMutation = useMutation(
    (playerId: number) => apiService.removeWatchedPlayer(playerId, 'MY'),
    {
      onSuccess: () => {
        queryClient.invalidateQueries(['watched-players', 'MY']);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Мои футболисты</h1>
          <p className="text-gray-600">
            Игроки, которых вы ведёте. Нажмите на игрока для полной аналитики.
          </p>
        </div>
          ...
          <CardContent>
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-gray-500 text-center max-w-md">
                Откройте карточку игрока в любом турнире и нажмите кнопку «Мой» чтобы добавить.
              </p>
            </div>
          </CardContent>
        </Card>

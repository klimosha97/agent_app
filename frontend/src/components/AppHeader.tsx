...
export const AppHeader: React.FC = () => {
  ...
  const handleClearDatabase = async () => {
    if (!window.confirm('⚠️ Вы уверены, что хотите очистить ВСЕ таблицы?\n\nЭто удалит всех игроков из базы данных!')) {
      return;
    }

    setIsClearing(true);
    try {
      const result = await apiService.clearAllTables();
      alert(`✅ Таблицы очищены!\n\nУдалено:\n- Игроков: ${result.deleted.players_stats_raw}\n- Записей последнего тура: ${result.deleted.last_round_stats}`);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-4">
          ...
          <div className="flex items-center space-x-4">
            ...
            {/* Временная кнопка очистки БД */}
            <button
              onClick={handleClearDatabase}
              disabled={isClearing}
              className="px-3 py-1.5 bg-red-500 hover:bg-red-600 disabled:bg-red-300 text-white text-sm font-medium rounded-md transition-colors flex items-center space-x-1"
              title="Очистить все таблицы"
            >
              {isClearing ? (
                <>

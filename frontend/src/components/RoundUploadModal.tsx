/**
 * Модальное окно для загрузки данных за конкретный тур
 * Хранит данные отдельно от сезонных (period_type='ROUND')
 */
...
interface RoundUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
  tournamentId: number;
...
const LAST_ROUND_UPLOAD_KEY = 'football_stats_last_round_upload';

// Получить последнюю загрузку тура для турнира
const getLastRoundUpload = (tournamentId: number): LastRoundUploadInfo | null => {
  try {
    const data = localStorage.getItem(`${LAST_ROUND_UPLOAD_KEY}_${tournamentId}`);
...
// Сохранить последнюю загрузку тура
const saveLastRoundUpload = (info: LastRoundUploadInfo) => {
  localStorage.setItem(`${LAST_ROUND_UPLOAD_KEY}_${info.tournamentId}`, JSON.stringify(info));
};

export const RoundUploadModal: React.FC<RoundUploadModalProps> = ({
  isOpen,
  onClose,
  tournamentId,
  tournamentName,
  ...
  const lastUpload = getLastRoundUpload(tournamentId);
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="relative bg-white rounded-xl shadow-2xl max-w-2xl w-full">
          {/* Header - зеленый для отличия от сезонного */}
          <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-emerald-50 to-teal-50">
            ...
            <button
              ...
              className="p-2 text-gray-400 hover:text-gray-600 rounded-lg hover:bg-white/50 disabled:opacity-50"
            >
            ...
            {lastUpload && (
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                <div className="flex items-start">
                  <InformationCircleIcon className="w-6 h-6 text-emerald-600 mt-0.5 mr-3 flex-shrink-0" />
                  <div className="text-sm text-emerald-800">
                    <p className="font-semibold mb-1">Последняя загрузка тура:</p>
                    <p className="flex items-center">
                      <FlagIcon className="w-4 h-4 mr-1" />
                      Сезон {lastUpload.season}, тур {lastUpload.round}
                    </p>
                    <p className="flex items-center mt-1 text-emerald-600">
            ...
            <div className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl p-5">
              ...
              <div className="mb-5">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-2xl font-bold text-emerald-900">{season}</span>
                </div>
                <input
                  type="range"
                  min="2020"
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-2xl font-bold text-emerald-900">Тур {round}</span>
                </div>
                <input
                  type="range"
                  min="1"
              ...
              {/* Итоговая информация */}
              <div className="mt-4 p-3 bg-white/70 rounded-lg border border-emerald-300">
                <p className="text-sm text-emerald-900 font-medium text-center">
                  🏁 Будут записаны данные: <strong>Сезон {season}, тур {round}</strong>
                </p>
                <p className="text-xs text-emerald-600 text-center mt-1">
                  Данные хранятся отдельно от сезонной статистики
                </p>
              </div>
            </div>
      ...
      <style>{`
        ...
        .slider-emerald:focus::-webkit-slider-thumb {

...
class ExcelImportService:
    ...
    def get_tournament_from_filename(self, filename: str, db_patterns: Optional[Dict] = None) -> Optional[int]:
        """
        Определение турнира по имени файла.
        db_patterns: {tournament_id: file_pattern} из БД (если передан).
        Если не передан, используется дефолтный маппинг.
        """
        filename_lower = filename.lower()
        
        if db_patterns:
            for tournament_id, pattern in db_patterns.items():
                if pattern and pattern.lower() in filename_lower:
                    return tournament_id
            return None
        
        for tournament_id, patterns in self._default_patterns.items():
